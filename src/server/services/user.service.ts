 ...
 * @returns {Promise<IUserQueryWithHelper>}
 */
export const getUserByIdNumber = async (
  idNumber: string
): Promise<IUserQueryWithHelper> => {
  return UserModel.findOne({ idNumber });
};
...
export const updateUserById = async (
  ...
  if (
    updateBody.email &&
    (await UserModel.isEmailTaken(updateBody.email, id))
  ) {
    throw new ApiError(httpStatus.BAD_REQUEST, "Email already taken");
  }
