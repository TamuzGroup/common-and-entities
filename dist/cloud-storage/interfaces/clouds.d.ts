...
export interface IClouds {
    ...
    getDriveFiles(folderId?: string, folderPath?: string): GaxiosPromise<drive_v3.Schema$FileList> | Promise<AxiosResponse> | Promise<DropboxResponse<files.ListFolderResult> | {
        data: {
    ...
    deleteFile(fileId?: string): Promise<GaxiosResponse<void>> | Promise<AxiosResponse> | Promise<DropboxResponse<files.DeleteResult>>;
    downloadFile(fileId?: string, fileName?: string): GaxiosPromise<Readable> | Promise<DropboxResponse<files.FileMetadata> | null>;
    getFileData(fileId?: string, fileName?: string): Promise<GaxiosResponse<drive_v3.Schema$File>> | Promise<DropboxResponse<sharing.ListSharedLinksResult> | {
        data: {
            url: string;
        };
    }>;
